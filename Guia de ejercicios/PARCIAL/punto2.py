...
#a)
def especie(dinosaurios):
    especies = []
    for dinos in dinosaurios:
        if dinos["especie"] not in especies:
            especies.append(dinos["especie"])
    return len(especies)

Contar_especies= especie(dinosaurios)
print("Cantidad de especies:", Contar_especies)

def descubridores(dinosaurios):
    descubridores = []
    for dinos in dinosaurios:
        if dinos["descubridor"] not in descubridores:
            descubridores.append(dinos["descubridor"])
    return len(descubridores)
    

contar_descubridor= descubridores(dinosaurios)
print("Cantidad de descubridores:", contar_descubridor)

#c)
print("Dinosaurios que empeizan con la letra T:")
for dinos in dinosaurios:
...
for dinos in dinosaurios:
    ...
    if nombre.startswith(("A", "Q", "S")):
        pila_dinos.arrive(dinos)

print("Dinosaurios que comienzan con A, Q Y S Son:")
while pila_dinos.size() > 0:
    dinos = pila_dinos.attention()
    print(dinos["nombre"])